...
VARIANT_SPECS: dict[str, VariantSpec] = {
    ...
    # -- Feature Extractor only: the promoted optimizer rung ----------------
    #
    # PROVENANCE. This file is not hand-written. It is the instruction Vertex
    # AI Prompt Optimizer returned, composed onto A0's user template and tool
    # description by `optimizer.compose_prompt_file`:
    #
    #     VAIPO run ID   20260811T060312Z
    #     generated      2026-08-11 (surface: prompts.optimize, status:
    #                    optimized, returned in 90s)
    #     pack sha256    bc07745f69e93b497bde1e07a7c0652aee219c317d337df574
    #                    45720bddf443bf
    #     system sha16   569838504b393eee
    #     durable copy   artifacts/optimizer/feature_extractor/
    #                    20260811T060312Z.txt
    #
    # The provenance lives here and not as a header inside the .txt for a
    # mechanical reason: a prompt file may have no text before its first
    # section marker, and anything inside `=== system ===` is sent to the
    # model and feeds `system_prompt_sha`, which is part of the replay store
    # key. A header would either be rejected at load time or silently
    # re-key every recorded FE optimizer trace. The file is therefore
    # byte-identical to the durable copy, and
    # `tests/test_prompts.py::test_the_promoted_optimizer_pack_is_the_measured_file`
    # locks both digests to the ones in the published ablation artifact.
    #
    # The rung was scored on the core 28 (clean) and on the full 70 (12 of
    # its own few-shot items fall inside that split). The shipping figure is
    # the core-28 one; see the ladder's contamination note.
    "gemini_optimizer_v1": VariantSpec(
        output_mode="tool",
        model_role="gemini_candidate",
        rung="A4-optimizer",
        subagents=("feature_extractor",),
        description=(
            "A0's user template and tool description, with a system "
            "instruction generated by Vertex AI Prompt Optimizer (run "
            "20260811T060312Z) rather than written by hand. Promoted into "
            "the pack because it is the shipping arm for Feature Extractor "
            "and a shipping arm has to live where every other arm lives — "
            "readable, reviewable, and diffable against A0. Registered here "
            "at import time; `optimizer.installed_rung` no longer stages a "
            "temporary copy."
        ),
    ),
}
