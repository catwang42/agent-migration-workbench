...
**REPLAY — every number below comes from model calls recorded 2026-08-09T16:07:15+00:00 to 2026-08-12T12:58:33+00:00, not from a run just now.**
...
| --- | --- | --- | --- | --- |
| `quality_delta_pp` | `>= -2.0` | -2.50 pp [-5.36 pp, +0.36 pp] (paired n=70) | ci_lower = -5.357 | **FAIL** — parity not demonstrated |
...
| `cost_savings_pct` | `>= 30.0` | 56.1% [54.5%, 57.5%] | ci_lower = 54.54 | PASS |
| `latency_p95` | `<= claude_baseline_p95` | 7,388 ms [5,998 ms, 7,600 ms] (same-region probe, global) | ci_upper = 7600 | **FAIL** |
...
| Gate | Bound (gates.yaml) | Measured (95% confidence range) | Bound tested | Result |
| --- | --- | --- | --- | --- |
| `quality_delta_pp` | `>= -2.0` | +0.32 pp [-3.76 pp, +4.46 pp] (paired n=70) | ci_lower = -3.759 | **FAIL** — parity not demonstrated |
| `json_schema_validity` | `>= 0.99` | 1.000 [1.000, 1.000] | ci_lower = 1 | PASS |
...
| `cost_savings_pct` | `>= 30.0` | 64.8% [63.8%, 65.7%] | ci_lower = 63.78 | PASS |
| `latency_p95` | `<= claude_baseline_p95` | 8,906 ms [6,767 ms, 9,554 ms] (same-region probe, global) | ci_upper = 9554 | **FAIL** |
...
| Gate | Bound (gates.yaml) | Measured (95% confidence range) | Bound tested | Result |
| --- | --- | --- | --- | --- |
| `quality_delta_pp` | `>= -2.0` | +7.32 pp [+2.86 pp, +12.14 pp] (paired n=70) | ci_lower = 2.857 | PASS |
| `json_schema_validity` | `>= 0.99` | 1.000 [1.000, 1.000] | ci_lower = 1 | PASS |
| `groundedness_delta_pp` | `>= -1.0` | not evaluated — groundedness_delta_pp is inapplicable to query_rewriter by instrument design, not unmeasured by omission. Its output is a search plan, which makes no claim about the input that could be supported or unsupported by it, so there is nothing for a groundedness instrument to score. The gate is therefore not measured — and specifically not passed. No substitute rule is invented for it: a verdict over five of the six pre-agreed gates is not the verdict that was agreed, so the engine issues none | not evaluated | not evaluated |
| `shadow_agreement` | `>= 0.9` | 0.586 [0.471, 0.700] | ci_lower = 0.4714 | PASS (by pre-registered alt clause: adjudication 16W/2L) |
| `cost_savings_pct` | `>= 30.0` | 42.7% [40.3%, 45.3%] | ci_lower = 40.32 | PASS |
...
- `groundedness_delta_pp` — groundedness_delta_pp is inapplicable to query_rewriter by instrument design, not unmeasured by omission. Its output is a search plan, which makes no claim about the input that could be supported or unsupported by it, so there is nothing for a groundedness instrument to score. The gate is therefore not measured — and specifically not passed. No substitute rule is invented for it: a verdict over five of the six pre-agreed gates is not the verdict that was agreed, so the engine issues none
...
| Recording window | 2026-08-09T16:07:15+00:00 to 2026-08-12T12:58:33+00:00 |
| Region(s) | Claude `global`, Gemini + judge `global` (source: config/models.yaml region pin where set, else $CLAUDE_REGION / $REGION, falling back to the customer profile; claude-sonnet -> global, gemini-flash-current-capped -> global) |
...
- shadow_agreement is carried from the DEFAULT thinking configuration (gemini-flash-current), not measured on the capped configuration: the shadow campaign in shadow_current_query_rewriter.json, shadow_current_chunk_summarizer.json, shadow_current_feature_extractor.json predates the capped arms and there was no time to re-run it before freeze. Same model, same prompt bytes, same shadow slice; different reasoning budget. Every other gate on this scorecard is measured on the capped configuration itself.
...
- Arms assembled for the gemini-flash-current-capped scorecard: claude_baseline from phase2_n70_widened.json (full-70 widening) and each subagent's ship-current-capped rung from artifacts/results/ablation_*.json. Both sets were scored by the phase-2 scorer; this assembly copies them and recomputes nothing.
...
- latency_p95 sample size: n=10 calls per arm per subagent, both arms pinned to global, probed 2026-08-12T13:20:51+00:00. The incumbent's own p95 moved between the 2026-08-12T10:30:51+00:00 and 2026-08-12T13:20:51+00:00 probes of the same model in the same region (query_rewriter 3,471 -> 6,893 ms; chunk_summarizer 7,121 -> 5,355 ms; feature_extractor 5,528 -> 6,063 ms), which is the size of the noise this comparison sits on. Latency requires measurement on production infrastructure; demo-window figures are directional.
