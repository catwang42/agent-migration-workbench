...
!!! info "Copied verbatim from the repository"
    ...
    **Claude Sonnet 5 (via Vertex partner models)** re-scoring the same recorded outputs against the same rubrics as the gated **Gemini 2.5 Pro** judge. The two are never averaged and one is never substituted for the other. The outputs re-scored are **Claude Sonnet 5** <small><code>claude_baseline</code></small> and **Gemini 2.5 Flash (development generation)** <small><code>gemini_naive</code></small> / <small><code>gemini_tuned_v1</code></small> — this validates the *instrument*, and was not re-run against the deployment generation.
