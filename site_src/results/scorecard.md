...
!!! info "Copied verbatim from the repository"
    ...
    The gates in `config/gates.yaml` evaluated against the **deployment configuration** — Gemini 3.6 Flash with the reasoning budget minimised, the arms the scorecard recommends. Copied verbatim — every caveat below sits in the same cell as the figure it qualifies.
