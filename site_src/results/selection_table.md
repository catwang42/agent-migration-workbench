...
!!! info "Copied verbatim from the repository"
    ...
    Every ablation rung and gated arm, per subagent, at the n it was actually run on. Incumbent rows <small><code>claude_baseline</code></small> are **Claude Sonnet 5 (via Vertex partner models)**; every other row on this frozen artifact is **Gemini 2.5 Flash (development generation)** — the rung names <small><code>gemini_naive</code></small>, <small><code>gemini_tuned_v1</code></small> and <small><code>gemini_optimizer_v1</code></small> are *prompt* identities, not model identities. The deployment-generation rungs are tabulated in [module 05](../modules/05-adaptation-ladder.md#what-ships-the-deployment-generation). Rows are grouped by split because the two splits have different incumbents — never quote a core-28 figure against a full-70 one.
